...
go
if OBJECT_ID('usp_ThemCTphieumuon') is not null
 Drop proc usp_ThemCTphieumuon
go
create proc usp_ThemCTphieumuon @STTMuon nchar(10), @MaTaiLieu nchar(10), @MaPhieuMuon nchar(10),@HanTra DATE
as
begin
	if exists( select * from [PHIEU MUON] where MaPhieuMuon=@MaPhieuMuon)

	insert into [CHI TIET PHIEU MUON]

	values(@STTMuon,@MaTaiLieu,@HanTra)
end
go
---
---------------------tim kiem------------------------
   if OBJECT_ID('usp_SearchPhieuMuontheoMaPM') is not null
 Drop proc usp_SearchPhieuMuontheoMaPM
 go

 create proc usp_SearchPhieuMuontheoMaPM @Maphieumuon char(15)
 as
 begin
	select *
	from [PHIEU MUON] join [CHI TIET PHIEU MUON] on [CHI TIET PHIEU MUON].MaPhieuMuon=[PHIEU MUON].MaPhieuMuon
	where [PHIEU MUON].MaPhieuMuon=@Maphieumuon
 end
 go
   if OBJECT_ID('usp_SearchPhieuMuontheoMaDG') is not null
 Drop proc usp_SearchPhieuMuontheoMaDG
 go

 create proc usp_SearchPhieuMuontheoMaDG @Madocgia char(15)
 as
 begin
	select *
	from [PHIEU MUON] join [CHI TIET PHIEU MUON] on [CHI TIET PHIEU MUON].MaPhieuMuon=[PHIEU MUON].MaPhieuMuon
 ...
 go
 -----------------------------xoa--------------------------------------
  if OBJECT_ID('usp_Xoaphieumuon') is not null
 Drop proc usp_Xoaphieumuon
 go

 create proc usp_Xoaphieumuon @Maphieumuon char(15)
 as
 begin
...
if OBJECT_ID('usp_ThemCTphieutra') is not null
 Drop proc usp_ThemCTphieutra
go
create proc usp_ThemCTphieutra @STTPhieuTra nchar(10), @MaPhieuTra nchar(10), @MaDocGia nchar(10),@ngaytra date
as
begin
	if exists( select * from [PHIEU TRA] where MaPhieuTra=@MaPhieuTra)

	insert into [CHI TIET PHIEU TRA]

	values(@STTPhieuTra,@MaPhieuTra ,@Madocgia,@ngaytra)
end
go
-------------------------------------------------
...
begin
	...
	values(@Mapt,@MaPhieuMuon, @MaNVLapPhieuTra, @ngaylapphieutra)
end
end
-----------------------------TIM KIẾM------------------------------
   if OBJECT_ID('usp_SearchPhieuTratheoMaPT') is not null
 Drop proc usp_SearchPhieuTratheoMaPT
 go

 create proc usp_SearchPhieuTratheoMaPT @Maphieutra char(15)
 as
 begin
	select *
	from  [CHI TIET PHIEU TRA] join [PHIEU TRA] on [CHI TIET PHIEU TRA].MaPhieuTra=[PHIEU TRA].MaPhieuTra
	where [PHIEU TRA].MaPhieuTra=@Maphieutra
 end
 go
    if OBJECT_ID('usp_SearchPhieuTratheoMaPM') is not null
 Drop proc usp_SearchPhieuTratheoMaPM
 go

 create proc usp_SearchPhieuTratheoMaPM @Maphieumuon char(15)
 as
 begin
	select *
	from  [CHI TIET PHIEU TRA] join [PHIEU TRA] on [CHI TIET PHIEU TRA].MaPhieuTra=[PHIEU TRA].MaPhieuTra
 ...
 go
     if OBJECT_ID('usp_SearchPhieuTratheoMaDG') is not null
 Drop proc usp_SearchPhieuTratheoMaDG
 go

 create proc usp_SearchPhieuTratheoMaDG @Madocgia char(15)
 as
 begin
	select *
	from  [CHI TIET PHIEU TRA] join [PHIEU TRA] on [CHI TIET PHIEU TRA].MaPhieuTra=[PHIEU TRA].MaPhieuTra
 ...
 go
 -------------------------xóa-----------------------------------
   if OBJECT_ID('usp_Xoaphieutra') is not null
 Drop proc usp_Xoaphieutra
 go

 create proc usp_Xoaphieutra @Maphieutra char(15)
 as
 begin
